...
$result = $query->fetch(PDO::FETCH_OBJ);
    if(isset($response->status)){
        if($response->status == 'Paid' && $result->status == 0){
            ...
            $sql = "UPDATE vas_client_wallet as wallet join vas_client_wallet_types as walTyp on wallet.vas_client_wallet_type_id = walTyp.id SET wallet.credits = (wallet.credits + 19.63) WHERE wallet.vas_client_centre_id = ? AND walTyp.`key` = ?";
            ...
            $complete = $connect->prepare("UPDATE vas_transaction_analysis SET status = 1, contipayRef = ?, vas_client_level = ?, points = ?, action_id = ?  WHERE id = ?");
            $complete->execute(array($response->contiPayRef, $vas->vas_cos_id, $awardResult->pointAwarded,3,  $result->id));
